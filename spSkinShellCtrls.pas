...
{       Version 14.30                                               }
{                                                                   }
{       Copyright (c) 2000-2021 Almediadev                          }
{       ALL RIGHTS RESERVED                                         }
{                                                                   }
...
uses
  ...
  Menus, SkinMenus, spPngImageList, spEffBmp;

const
  SP_SID_IShellItemImageFactory = '{bcc18b79-ba16-442f-80c4-8a59c30c463b}';
  SP_IID_IShellItemImageFactory: TGUID = SP_SID_IShellItemImageFactory;
  SP_SID_IExtractImage        = '{BB2E617C-0920-11D1-9A0B-00C04FC2D6C1}';
  SP_IID_IImageList: TGUID= '{46EB5926-582E-4017-9FDF-E8998DAA0950}';
  SP_IID_IExtractImage: TGUID = SP_SID_IExtractImage;
  SP_CLSID_ShellItem: TGUID = (
    D1:$43826d1e; D2:$e718; D3:$42ee; D4:($bc,$55,$a1,$e2,$61,$c3,$7b,$fe));
  SP_IEIT_PRIORITY_NORMAL = $10000000;
  SP_IEIFLAG_OFFLINE = $0008;
  SP_IEIFLAG_QUALITY = $0200;
  SP_IEIFLAG_ORIGSIZE = $0040;
...
type
  ...
  SP_IShellItem = interface(IUnknown)
    ...
    function Compare(const psi: SP_IShellItem; hint: DWORD;
      var piOrder: Integer): HResult; stdcall;
   end;

  SP_IShellItemImageFactory = interface(IUnknown)
    [SP_SID_IShellItemImageFactory]
    function GetImage(size: TSize; flags: UINT; out phbm: HBITMAP): HRESULT; stdcall;
  end;

  SP_IExtractImage = interface(IUnknown)
    [SP_SID_IExtractImage]
...
type
  ...
  TspThumbnailState = set of (spthmbIconLoaded, spthmbThumbnailLoaded);

  TspThumbnailsThread = class(TThread)
  protected
    FLV: TspCustomShellListView;
    FStartIndex, FStep: Integer;
    FIconOnly: Boolean;
    procedure Execute; override;
    procedure LoadThumbnails;
    procedure LoadThumbnail(AFolder: TspShellFolder; AIndex: Integer);
    procedure LoadIcon(AFolder: TspShellFolder; AIndex: Integer);
  public
    constructor CreateEx(AFLV: TspCustomShellListView; AStartIndex, AStep: Integer; AIconOnly: Boolean);
  end;
  ...
  public
    ...
    FThumbnail: TspBitmap;
    FThumbnailState: TspThumbnailState;
...
{ TShellTreeView }
  ...
  TspThumbnailOptions = class(TPersistent)
  private
    FHeight: Integer;
    FEnabled: Boolean;
    FWidth: Integer;
  ...
  private
    FThumbnailOptions: TspThumbnailOptions;
    ...
    procedure SetPath(const Value: string);
  protected
    procedure UpdateThumbnail(AFolder: TspShellFolder);
    procedure NewAdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure SetRootDirectly(const Value: TRoot);
    ...
    property ThumbnailOptions: TspThumbnailOptions read
...
{ TShellListView }
  ...
  public
    ...
    FileName: String;
    DSF: TspDynamicSkinForm;
    FileListViewPanel, ButtonsPanel,
    BottomPanel, ClientPanel: TspSkinPanel;
    ...
    ToolPanel: TspSkinToolBar;
    NewFolderToolButton, UpToolButton, BackToolButton: TspSkinButton;
    StyleToolButton: TspSkinMenuButton;
    StylePopupMenu: TspSkinPopupMenu;
    IconMenuItem, SmallIconMenuItem, ReportMenuItem, ListMenuItem,
    ThumbnailsItem, BigThumbnailsItem, VeryBigThumbnailsItem: TMenuItem;
    OnFolderChange: TNotifyEvent;
    CheckFileExists: Boolean;
    constructor CreateEx(AOwner: TComponent; ASaveMode: Boolean; ACtrlSkinData: TspSkinData;
    AToolButtonsTransparent: Boolean; AToolBarImageList: TCustomImageList; AShowThumbnails: Boolean);
    destructor Destroy; override;
    ...
    procedure ThumbnailsItemClick(Sender: TObject);
    procedure BigThumbnailsItemClick(Sender: TObject);
    procedure VeryBigThumbnailsItemClick(Sender: TObject);
    procedure InitHistory;
    procedure CreateShellButtons;
    procedure ShellButtonClick(Sender: TObject);
  end;

  TspShellListViewStyle = (spvsIcon, spvsSmallIcon, spvsList, spvsReport,
  spvsThumbnails, spvsBigThumbnails, spvsVeryBigThumbnails);
  ...
  protected
    ...
    procedure Change;
  public
    ListViewStyle: TspShellListViewStyle;
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function Execute: Boolean;
  ...
  public
    ...
    DSF: TspDynamicSkinForm;
    FileListViewPanel,
    BottomPanel, ButtonsPanel, ClientPanel: TspSkinPanel;
    FLVHScrollBar, FLVVScrollBar: TspSkinScrollBar;
    FileListView: TspSkinFileListView;
    FileNameEdit: TspSkinEdit;
    FilterComboBox: TspSkinFilterComboBox;
    ...
    constructor CreateEx(AOwner: TComponent; ASaveMode: Boolean;
                         ...
                         AToolButtonsTransparent: Boolean;
                         AToolBarImageList: TCustomImageList; AShowThumbnails: Boolean);
    destructor Destroy; override;
    procedure FLVChange(Sender: TObject; Item: TListItem; Change: TItemChange);
    procedure FLVPathChange(Sender: TObject);
  ...
  public
    ...
    //
    constructor CreateEx(AOwner: TComponent; ACtrlSkinData: TspSkinData; AToolButtonsTransparent: Boolean;
    AToolBarImageList: TCustomImageList; AShowThumbnails: Boolean);
    destructor Destroy; override;
    procedure FLVChange(Sender: TObject; Item: TListItem; Change: TItemChange);
    procedure FLVPathChange(Sender: TObject);
  ...
  public
    ...
    ShellBox: TspSkinShellComboBox;
    OpenButton, CancelButton: TspSkinButton;
    PlayButton, StopButton: TspSkinButton;
    Drivelabel, OpenFileLabel, FileTypeLabel: TspSkinStdLabel;
    ToolPanel: TspSkinToolBar;
    NewFolderToolButton, UpToolButton, BackToolButton: TspSkinButton;
    StyleToolButton: TspSkinMenuButton;
...
const
 ...
 var
   SP_SHCreateItemFromIDList: function(pidl: PItemIdList; const riid: TIID; out ppvOut): HResult; stdcall;
   SP_SHGetImageList: function (iImageList: integer; const riid: TGUID; var ppv: Pointer): hResult; stdcall;
   SP_SHGetKnownFolderIDList: function(const rfid: TGUID; dwFlags: DWORD; hToken: THandle; var ppidl: PItemIDList ): HResult; stdcall;
   
implementation

{$R spSkinShellCtrls}

uses ShellAPI, ComObj, TypInfo, Consts, Math, spConst, Masks, {$IFNDEF VER130} MaskUtils,{$ENDIF}spmessages,
     spUtils;
...
begin
  ...
  {$ELSE}
  HR := AShellFolder2.MapNameToSCID(PWideChar(AColumnID), pscid);
  {$ENDIF}
  Result := HR = S_OK;
end;
...
constructor TspThumbnailsThread.CreateEx(AFLV: TspCustomShellListView; AStartIndex, AStep: Integer; AIconOnly: Boolean);
begin
...
procedure TspThumbnailsThread.Execute;
begin
  LoadThumbnails;
end;

procedure TspThumbnailsThread.LoadThumbnails;
var
  I: Integer;
...
procedure TspThumbnailsThread.LoadThumbnail(AFolder: TspShellFolder; AIndex: Integer);
var
  FThumbSize: TSize;
  FHBitmap: HBitmap;
  FShellItem: SP_IShellItem;
  FItemImageFactory: SP_IShellItemImageFactory;
  FShellFolder: IShellFolder;
  FTempPidl: PItemIDList;
  FExtractor: SP_IExtractImage;
  FPriority: Cardinal;
  FFlags: Cardinal;
  FPathBuffer: array [0..1024] of WideChar;

procedure LoadDefaultIcon;
begin
  if AFolder.FThumbnailIndex = -1 then
    AFolder.FThumbnailIndex := AFolder.ImageIndex(True);
  if not (spthmbIconLoaded in AFolder.FThumbnailState) then
  begin
    AFolder.FThumbnailState := AFolder.FThumbnailState + [spthmbIconLoaded];
    if FLV.HandleAllocated and not FLV.FStopFetch  then
...
begin
  ...
  if FLV.FIsVistaOrLater and
    (@SP_SHCreateItemFromIDList <> nil) and
    (SP_SHCreateItemFromIDList(AFolder.FFullPIDL, SP_CLSID_ShellItem, FShellItem) = S_Ok) and
    (FShellItem.QueryInterface(SP_IID_IShellItemImageFactory, FItemImageFactory) = S_OK) and
  ...
  begin
    FShellFolder := AFolder.ParentShellFolder;
    FTempPidl := CopyPidl(AFolder.FPIDL);
    try
      if (FShellFolder <> nil) and
         (FShellFolder.GetUIObjectOf(0, 1, FTempPidl,
             SP_IID_IExtractImage, nil, FExtractor) = S_OK) then
      begin
        FPriority := SP_IEIT_PRIORITY_NORMAL;
        FFlags := SP_IEIFLAG_OFFLINE or SP_IEIFLAG_QUALITY or SP_IEIFLAG_ORIGSIZE;
        FExtractor.GetLocation(FPathBuffer, 255, FPriority, FThumbSize, 32, FFlags);
        if FExtractor.Extract(FHBitmap) = S_OK then
        begin
         AFolder.FThumbnailHBitmap := FHBitmap;
         if FLV.HandleAllocated and not FLV.FStopFetch then
            ListView_ReDrawItems(FLV.Handle, AIndex, AIndex);
...
procedure TspThumbnailsThread.LoadIcon(AFolder: TspShellFolder; AIndex: Integer);
var
  FThumbSize: TSize;
  FHBitmap: HBitmap;
  FShellItem: SP_IShellItem;
...
begin
  if FLV.FStopFetch or (spthmbThumbnailLoaded in AFolder.FThumbnailState) or
...
constructor TspThumbnailOptions.Create(AOwner: TPersistent);
begin
  inherited Create;
...
procedure TspThumbnailOptions.SetValues(AEnabled: Boolean; AWidth, AHeight: Integer);
begin
  FEnabled := AEnabled;
  FWidth := AWidth;
  FHeight := AHeight;
...
procedure TspThumbnailOptions.Assign(Source: TPersistent);
begin
  if Source is TspThumbnailOptions then
    with TspThumbnailOptions(Source) do
    begin
      Self.FHeight := Height;
      Self.FWidth := Width;
...
procedure TspThumbnailOptions.Changed;
begin
  DoChange;
end;

procedure TspThumbnailOptions.DoChange;
begin
  if Assigned(FOnChange) and FEnabled then
    FOnChange(Self);
end;

procedure TspThumbnailOptions.DoEnabledChange;
begin
  if Assigned(FOnEnabledChange) then
    FOnEnabledChange(Self);
end;

procedure TspThumbnailOptions.SetHeight(const Value: Integer);
begin
...
procedure TspThumbnailOptions.SetEnabled(const Value: Boolean);
begin
...
procedure TspThumbnailOptions.SetWidth(const Value: Integer);
begin
...
begin                 
  ...
  FIsVistaOrLater := IsVistaOS;
  FThumbnailOptions := TspThumbnailOptions.Create(Self);
...
procedure TspCustomShellListView.ClearThreads;
var
  I: Integer;
begin
  FStopFetch := True;
  ...
  begin
    ...
    begin
      TspThumbnailsThread(FThumbsThreadList[I]).Terminate;
      TspThumbnailsThread(FThumbsThreadList[I]).WaitFor;
      TspThumbnailsThread(FThumbsThreadList[I]).Free;
    end;
...
function TspCustomShellListView.GetThreadCount: Integer;
begin
  Result := 4;
...
function TspCustomShellListView.IsThumbnailView: Boolean;
begin
  Result := FIsVistaOrLater and (ViewStyle = vsIcon) and FThumbnailOptions.Enabled;
end;

procedure TspCustomShellListView.UpdateThumbnails;
begin
  ...
  begin
    ...
    if not Self.DrawSkin then DrawSkin := True;
  end
  else
  begin
    DoubleBuffered := not IsWXP;
...
procedure TspCustomShellListView.OnThumbnailOptionsEnabledChange(Sender: TObject);
begin
...
procedure TspCustomShellListView.OnThumbnailOptionsChange(Sender: TObject);
begin
  UpdateThumbnails;
  RootChanged;
end;

 procedure TspCustomShellListView.NewAdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
      State: TCustomDrawState; Stage: TCustomDrawStage; var DefaultDraw: Boolean);
 begin
   if FStopFetch or (Item.Index > Self.FFolders.Count - 1) then Exit;
   inherited;
 end;

procedure TspCustomShellListView.UpdateThumbnail(AFolder: TspShellFolder);
var
  FTempBitmap: TBitmap;
begin
  if AFolder <> nil then
  begin
    if (AFolder.FThumbnailHBitmap > 0) and not (spthmbThumbnailLoaded in AFolder.FThumbnailState)
    then
    begin
      if AFolder.FThumbnail.Width * AFolder.FThumbnail.Height <> 0 then
      begin
        AFolder.FThumbnail.Free;
        AFolder.FThumbnail := TspBitmap.Create;
      end;
      AFolder.FThumbnail.AlphaBlend := True;
      FTempBitmap := TBitmap.Create;
      FTempBitmap.Handle := AFolder.FThumbnailHBitmap;
      AFolder.FThumbnail.SetSize(FTempBitmap.Width, FTempBitmap.Height);
      AFolder.FThumbnail.SetAlpha(0);
      BitBlt(AFolder.FThumbnail.Canvas.Handle, 0, 0, FTempBitmap.Width, FTempBitmap.Height,
         FTempBitmap.Canvas.Handle, 0, 0, SRCCOPY);
      AFolder.FThumbnail.CheckingAlphaBlend;
      FTempBitmap.Free;
      AFolder.FThumbnailState := AFolder.FThumbnailState + [spthmbThumbnailLoaded];
      AFolder.FThumbnailHBitmap := 0;
    end
    else
    if (AFolder.FIconHBitmap > 0) and not (spthmbIconLoaded in AFolder.FThumbnailState) and
       not (spthmbThumbnailLoaded in AFolder.FThumbnailState)  then
    begin
      AFolder.FThumbnail.AlphaBlend := True;
      FTempBitmap := TBitmap.Create;
      FTempBitmap.Handle := AFolder.FIconHBitmap;
      AFolder.FThumbnail.SetSize(FTempBitmap.Width, FTempBitmap.Height);
      AFolder.FThumbnail.SetAlpha(0);
      BitBlt(AFolder.FThumbnail.Canvas.Handle, 0, 0, FTempBitmap.Width, FTempBitmap.Height,
         FTempBitmap.Canvas.Handle, 0, 0, SRCCOPY);
...
procedure TspCustomShellListView.DrawThumbnailImage(AItem: TListItem;
  ACanvas: TCanvas; ARect: TRect);
var
  AFolder: TspShellFolder;
  CX, CY, W, H: Integer;
begin
  ...
  if not FIsVistaOrLater then Exit;
  ...
  begin
    ...
    then
    begin
      if AItem.Selected and FThumbnailOptions.DrawSelectionFrame then
      with ACanvas do
      begin
        Brush.Style := bsClear;
        if (FSD = nil) or (FSD.Empty) then
          Pen.Color := clHighLight
        else
          Pen.Color := FSD.SkinColors.cHighLight;
        Rectangle(ARect.Left, ARect.Top, ARect.Right, ARect.Bottom);
      end;
      CX := ARect.Left + RectWidth(ARect) div 2 - FThumbnail.Width div 2;
      CY := ARect.Top + RectHeight(ARect) div 2 - FThumbnail.Height div 2;
      FThumbnail.Draw(ACanvas, CX, CY);
    end;
  end;
end;
...
function TspCustomShellListView.GetSelectedPath: String;
const
  CSIDL_MYVIDEO = $0e;
  CSIDL_MYPICTURES = $27;
  CSIDL_MYMUSIC = $0d;
...
begin
  ...
  then
    begin
      ...
      if (SortColumn = 0) or
         ((SortColumn <> 0) and (Pos({$IFDEF VER240_UP}FormatSettings.{$ENDIF}TimeSeparator, TspShellFolder(Item1).Details[4]) <> 0))
      then
      begin
      if TspShellFolder(Item1).FDetailsEx then
      begin
       case SortColumn of
      0:
        begin
...
begin
  ...
  if HandleAllocated then
    UpdateThumbnails;
  if not FRootLoaded then CreateRoot;
  RootChanged;
end;
...
var
  ...
  W, I: Integer;
  R: TRect;
  FThumbsThread: TspThumbnailsThread;
...
begin
  ...
  then
   ...
   begin
     ...
     begin
       FThumbsThread := TspThumbnailsThread.CreateEx(Self, I, FThreadCount, False);
       FThumbsThreadList.Add(FThumbsThread);
       FThumbsThread.Resume;
     end;
     for I := 0 to FThreadCount - 1 do
     begin
       FThumbsThread := TspThumbnailsThread.CreateEx(Self, I, FThreadCount, True);
       FThumbsThreadList.Add(FThumbsThread);
       FThumbsThread.Resume;
     end;
   end;
...
procedure TspCustomShellListView.SetRootDirectly(const Value: TRoot);
begin
  FOldRoot := FRoot;
  FRoot := Value;
  CreateRoot;
...
var
  ...
  J: Integer;
begin
  if FStopFetch or (Item.Index > Items.Count - 1) or (Item.Index > FFolders.Count - 1) or (Item.Index < 0) then
  begin
    Result := False;
    Exit;
  end;
...
begin
  ...
  try
    ...
    DSF.MinClientWidth := 300;
    DSF.MinClientHeight := 300;
    //
...
begin
  ...
  BackToolButton := TspSkinButton.Create(Self);
  ...
  begin
    ...
    ShowHint := True;
    CanFocused := False;
    ImageList := ToolPanel.Images;
    Transparent := AToolButtonsTransparent;
  end;

  UpToolButton := TspSkinButton.Create(Self);
  ...
  NewFolderToolButton := TspSkinButton.Create(Self);
  with NewFolderToolButton do
  begin
    Left := Self.Width;
    Parent := ToolPanel;
  ...
  if IsVistaOS and AShowThumbnails then
  begin
    ThumbnailsItem := TMenuItem.Create(Self);
    with ThumbnailsItem do
    begin
      if ResStrData <> nil
      then
        Caption := ResStrData.GetResStr('MSG_LV_THUMBNAILS')
      else
        Caption := SP_MSG_LV_THUMBNAILS;
      if Caption = '' then Caption := SP_MSG_LV_THUMBNAILS;
      GroupIndex := 1;
      RadioItem := True;            
      OnClick := ThumbnailsItemClick;
    end;
    StylePopupMenu.Items.Add(ThumbnailsItem);

    BigThumbnailsItem := TMenuItem.Create(Self);
    with BigThumbnailsItem do
    begin
      if ResStrData <> nil
      then
        Caption := ResStrData.GetResStr('MSG_LV_BIGTHUMBNAILS')
      else
        Caption := SP_MSG_LV_BIGTHUMBNAILS;
      if Caption = '' then Caption := SP_MSG_LV_BIGTHUMBNAILS;
      GroupIndex := 1;
      RadioItem := True;            
      OnClick := BigThumbnailsItemClick;
    end;
    StylePopupMenu.Items.Add(BigThumbnailsItem);

    VeryBigThumbnailsItem := TMenuItem.Create(Self);
    with VeryBigThumbnailsItem do
    begin
      if ResStrData <> nil
      then
        Caption := ResStrData.GetResStr('MSG_LV_VERYBIGTHUMBNAILS')
      else
        Caption := SP_MSG_LV_VERYBIGTHUMBNAILS;
      if Caption = '' then Caption := SP_MSG_LV_VERYBIGTHUMBNAILS;
      GroupIndex := 1;
      RadioItem := True;
      OnClick := VeryBigThumbnailsItemClick;
    end;
    StylePopupMenu.Items.Add(VeryBigThumbnailsItem);
  end;

  StyleToolButton := TspSkinMenuButton.Create(Self);
  ...
  ButtonsPanel := TspSkinPanel.Create(Self);
  ...
  ClientPanel := TspSkinPanel.Create(Self);
  with ClientPanel do
  begin
    Parent := Self;
    Align := alClient;
...
begin
  if @SP_SHCreateItemFromIDList <> nil then
    SP_SHGetKnownFolderIDList(FOLDERID_LIBRARIES, 0, 0, Result)
  else
    Result := nil;
end;

procedure TspOpenDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TspShellFolder;
  NewPIDL: PItemIDList;
begin
  if (TspSkinButton(Sender).Tag = Ord(rfPersonal)) and IsVistaOS then
  begin
    NewPIDL := GetIdList_Libraries;
    if NewPIDL <> nil then
      FileListView.SetPathFromID(NewPIDL)
    else
      FileListView.SetRootDirectly(GetEnumName(TypeInfo(TspRootFolder), TspSkinButton(Sender).Tag));
  end
  else
    FileListView.SetRootDirectly(GetEnumName(TypeInfo(TspRootFolder), TspSkinButton(Sender).Tag));
  F := FileListView.FRootFolder;
  ShellBox.SetPathFromID(F.AbsoluteID);
end;

procedure TspOpenDlgForm.CreateShellButtons;

  procedure CreateBtn(Folder: TspRootFolder);
  var
    FButton: TspSkinButton;
    NewPIDL: PItemIDList;
  begin
    FButton := TspSkinButton.Create(Self);
    FButton.CanFocused := False;
    FButton.FShellImage := True;
    FButton.Transparent := True;
    FButton.SkinDataName := 'resizetoolbutton';
    FButton.Height := 70;
    FButton.Layout := blGlyphTop;
    FButton.SkinData := CtrlSD;
    if (Folder = rfPersonal) and IsVistaOS then
    begin
      NewPIDL := GetIdList_Libraries;
      if NewPIDL = nil then
      ...
      SHGetSpecialFolderLocation(0, nFolder[Folder], NewPIDL);
    FButton.Caption := GetDisplayName(DesktopShellFolder, NewPIDL, SHGDN_NORMAL);
    FButton.ImageIndex := GetShellImage(newPIDL, True, False);
    FButton.ImageList := FButtonImages;
    FButton.Align := alTop;
    FButton.Tag := Ord(Folder);
    FButton.OnClick := ShellButtonClick;
    FButton.Parent := ButtonsPanel;
  end;
...
begin
  ...
  CreateBtn(rfDesktop);
  CreateBtn(rfPersonal);
  CreateBtn(rfMyComputer);
  CreateBtn(rfNetwork);
end;
...
procedure TspOpenDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
  BigThumbnailsItem.Checked := True;
end;

procedure TspOpenDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
  VeryBigThumbnailsItem.Checked := True;
end;

procedure TspOpenDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
  ThumbnailsItem.Checked := True;
end;
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TspOpenDlgForm.CreateEx(Application, FSaveMode,
    CtrlSkinData, FToolButtonsTransparent, FToolBarImageList, FShowThumbnails);
  ...
  try
    ...
    DSF.MinWidth := FDialogMinWidth;
    DSF.MinHeight := FDialogMinHeight;
    DSF.MinClientHeight := 350;
    DSF.MinClientWidth := 550;
    // alpha blend
    DSF.AlphaBlend := Self.AlphaBlend;
    DSF.AlphaBlendAnimation := AlphaBlendAnimation;
    DSF.AlphaBlendValue := Self.AlphaBlendValue;
    ...
    //
    if (not FShowThumbnails or not IsVistaOS) and
    ((ListViewStyle = spvsThumbnails) or
    (ListViewStyle = spvsBigThumbnails) or
    (ListViewStyle = spvsVeryBigThumbnails))
    then
      ListViewStyle := spvsList;
    //
    ...
    BottomPanel.SkinData := FCtrlFSD;
    ToolPanel.SkinData := FCtrlFSD;
    ClientPanel.SkinData := FCtrlFSD;
    ButtonsPanel.SkinData := FCtrlFSD;
    //
    ...
    OpenFileLabel.Height := FileNameEdit.Height - 1;
    FileTypeLabel.SkinData := FCtrlFSD;
    FileTypeLabel.Height := FilterComboBox.Height - 1;
    //
    ...
    FileNameEdit.Width := BottomPanel.Width - 170;
    ...
    begin
      case FileListView.ThumbnailOptions.Width of
        96: ListViewStyle := spvsThumbnails;
        128: ListViewStyle := spvsBigThumbnails;
        256: ListViewStyle := spvsVeryBigThumbnails;
      end;
    end
    else
      case FileListView.ViewStyle of
        vsIcon:
          ListViewStyle := spvsIcon;
        vsSmallIcon:
          ListViewStyle := spvsSmallIcon;
        vsReport:
          ListViewStyle := spvsReport;
        vsList:
          ListViewStyle := spvsList;
      end;
...
begin
 ...
 IconMenuItem := TMenuItem.Create(Self);
  ...
  StretchButton := TspSkinButton.Create(Self);
...
procedure TspOpenPictureDlgForm.CreateShellButtons;
...
procedure TspOpenPictureDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TspShellFolder;
  NewPIDL: PItemIDList;
begin
...
begin
  ...
  then
    ...
    begin
      ...
      ScrollBox.HScroll(0);
      ScrollBox.VScroll(0);
      Image.SetBounds(0, 0, Image.Picture.Width, Image.Picture.Height);
      ScrollBox.Realign;
      ScrollBox.Parent.Realign;
...
procedure TspOpenPictureDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenPictureDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenPictureDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
begin
  ...
  then
    begin
      try
        Image.Picture.LoadFromFile(FileListView.GetSelectedFile);
      finally
        {$IFDEF VER130}
        if not Image.Stretch
        {$ELSE}
        if not Image.Proportional
        {$ENDIF}
        then
        begin
          ScrollBox.HScroll(0);
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TspOpenPictureDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent,
  FToolBarImageList, FShowThumbnails);
  FDlgFrm.CheckFileExists := FCheckFileExists;
...
begin
  ...
  with PreviewDSF do
  begin
    PreviewDSF.BorderIcons := PreviewDSF.BorderIcons - [biSystemMenu];
    SkinData := PreviewSkinData;
    PreviewMode := True;
  end;
...
procedure TspOpenSkinDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TspShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TspOpenSkinDlgForm.CreateShellButtons;
...
procedure TspOpenSkinDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenSkinDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenSkinDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
begin
  ...
  then
    begin
      ...
      then
        PreviewSkinData.LoadFromCompressedFile(FileListView.GetSelectedFile)
      else
      if FilterComboBox.ItemIndex = FUnCompressedFilterIndex - 1
      then
        PreviewSkinData.LoadFromFile(FileListView.GetSelectedFile);
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TspOpenSkinDlgForm.CreateEx(Application, CtrlSkinData, FToolButtonsTransparent, FToolBarImageList, FShowThumbnails);
  with FDlgFrm do
  try
    Caption := Self.Title;
    DSF.BorderIcons := [];
...
procedure TspOpenPreviewDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TspShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TspOpenPreviewDlgForm.CreateShellButtons;
...
procedure TspOpenPreviewDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenPreviewDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenPreviewDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
begin
  ...
  FDlgFrm := TspOpenPreviewDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData,
   FToolButtonsTransparent, FToolBarImageList, FShowThumbnails);
  FDlgFrm.OnFolderChange := FOnFolderChange;
  FDlgFrm.CheckFileExists := FCheckFileExists;
...
Drivelabel := TspSkinStdLabel.Create(Self);
  ...
  PlayButton := TspSkinButton.Create(Self);
  with PlayButton do
  begin
    Align := alLeft;
    Parent := SoundPanel;
    ...
    CanFocused := False;
    ImageList := SoundPanel.Images;
    Transparent := AToolButtonsTransparent;
  end;

  StopButton := TspSkinButton.Create(Self);
  with StopButton do
  begin
    Align := alLeft;
    Parent := SoundPanel;
...
procedure TspOpenSoundDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TspShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TspOpenSoundDlgForm.CreateShellButtons;
...
procedure TspOpenSoundDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenSoundDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TspOpenSoundDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
begin
  ...
  try
   ...
   // alphablend
    ...
    NewFolderToolButton.SkinData := FCtrlFSD;
    //
    PlayButton.SkinData := FCtrlFSD;
    StopButton.SkinData := FCtrlFSD;
    //
    FLVHScrollBar.SkinData := FCtrlFSD;
    FLVVScrollBar.SkinData := FCtrlFSD;
    FileNameEdit.SkinData := FCtrlFSD;
...
initialization
  ...
  if Shell32DLL <> 0 then begin
    SP_SHCreateItemFromIDList := GetProcAddress(Shell32DLL, 'SHCreateItemFromIDList');
    SP_SHGetImageList := GetProcAddress(Shell32DLL, 'SHGetImageList');
    SP_SHGetKnownFolderIDList := GetProcAddress(Shell32DLL, 'SHGetKnownFolderIDList');
  end;
