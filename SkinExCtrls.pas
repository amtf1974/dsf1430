...
type
 ...
 published
  ...
  protected
    ...
    procedure DrawItem(Index: Integer; Cnvs: TCanvas; AAlternateColor: Boolean);
    procedure SkinDrawItem(Index: Integer; Cnvs: TCanvas; AAlternateColor: Boolean);
    procedure DefaultDrawItem(Index: Integer; Cnvs: TCanvas; AAlternateColor: Boolean);
...
procedure TspSkinOfficeListBox.EnableScrollTimer(Value: Integer);
begin
...
procedure TspSkinOfficeListBox.StopScrollTimer;
begin
  if FTimerMode <> 0 then
  begin
    FTimerMode := 0;
...
procedure TspSkinOfficeListBox.WMTimer;
begin
  inherited;
...
procedure TspSkinOfficeListBox.SetAlternateRow(Value: Boolean);
begin
...
begin
  ...
  then
    ...
    begin
      H := H + Height - RectHeight(RealClientRect);
    end;
  Result := H;  
end;
...
begin
  if FIndex <> -1
  then
    SkinDrawItem(Index, Cnvs, AAlternateColor)
  else
    DefaultDrawItem(Index, Cnvs, AAlternateColor);
end;
...
begin
  ...
  begin
    if FItems[I].IsVisible
    then
      begin
        DrawItem(I, B.Canvas, Alternate);
        Alternate := not Alternate;
        if FItems[I].Header then Alternate := False;
      end;
  end;
...
procedure TspSkinOfficeListBox.SkinDrawItem;
var
...
begin
  ...
  //
  if FAlternateRow and AAlternateColor and not FItems[Index].Header
  then
    begin
      Cnvs.Brush.Style := bsSolid;
      Cnvs.Brush.Color := GetAlternateColor(FSD.SkinColors.cWindow);
      Cnvs.FillRect(R);
    end;
  //
  InflateRect(R, -2, -2);
    ...
    then
     ...
     begin
      ...
      then
        ...
        then
         begin
           DrawImageAndText2(Cnvs, R, 0, 5, blGlyphLeft,
    ...
    begin
     ...
     then
      ...
      then
        ...
        then
         begin
           DrawImageAndText2(Buffer.Canvas, R, 0, 5, blGlyphLeft,
             Caption, FImageIndex, FImages,
             False, Enabled, False, 0);
         end
       else
...
procedure TspSkinOfficeListBox.DefaultDrawItem;
var
  R, R1, CR: TRect;
  C, FC: TColor;
  TX, TY: Integer;
...
begin
  ...
  then
    begin
      Cnvs.Brush.Style := bsSolid;
      Cnvs.Brush.Color := GetAlternateColor(clWindow);
    end;
...
procedure TspSkinOfficeListBox.InitItemIndex(Value: Integer);
var
  I: Integer;
begin
  if Value < 0 then
...
function TspSkinCustomOfficeComboBox.GetAlternateRow: Boolean;
begin
  Result := FListBox.AlternateRow;
end;

procedure TspSkinCustomOfficeComboBox.SetAlternateRow(Value: Boolean);
begin
...
procedure TspSkinCustomOfficeComboBox.InitItemIndex(Value: Integer);
begin
...
begin
  ...
  then
    ...
    begin
      ...
      then
        begin
          Frame3D(C, R1, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
          C.Brush.Color := SP_BTNDOWNCOLOR;
          C.FillRect(R1);
        end
      else
        if MouseIn
        then
          begin
            Frame3D(C, R1, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
            C.Brush.Color := SP_BTNACTIVECOLOR;
            C.FillRect(R1);
          end
        else
          begin
       ...
       if FShowItemText and FShowItemTitle and (Items[Index].Title <> '')
        ...
        then
          begin
            if FShowItemText
            then
              DrawImageAndText2(Buffer.Canvas, R1, 0, 5, blGlyphLeft,
      ...
      begin
        ...
        then
          begin
            if FShowItemText
            then
              DrawImageAndText2(B.Canvas, R, 0, 5, blGlyphLeft,
...
procedure TspSkinHorzListBox.EnableScrollTimer(Value: Integer);
begin
  if FTimerMode = 0 then
  begin
    FTimerMode := Value;
...
procedure TspSkinHorzListBox.StopScrollTimer;
begin
  if FTimerMode <> 0 then
  begin
    FTimerMode := 0;
...
procedure TspSkinHorzListBox.WMTimer;
begin
  inherited;
  if Message.TimerID = 1 then
  begin
...
begin
  ...
  then
    ...
    begin
      W := W + Width - RectWidth(RealClientRect);
    end;
  Result := W;
end;
...
procedure TspSkinHorzListBox.InitItemIndex(Value: Integer);
var
  I: Integer;
begin
  if Value < 0 then
...
procedure TspSkinGridView.EnableScrollTimer(Value: Integer);
begin
  if FTimerMode = 0 then
  begin
    FTimerMode := Value;
...
procedure TspSkinGridView.StopScrollTimer;
begin
  if FTimerMode <> 0 then
  begin
    FTimerMode := 0;
...
procedure TspSkinGridView.WMTimer;
begin
  inherited;
  if Message.TimerID = 1 then
  begin
