...
type
 {$IFDEF VER200}
  TNotifyIconDataEx = TNotifyIconData;
  {$ELSE}
  ...
  protected
    ...
    procedure Notification(AComponent: TComponent; Operation: TOperation);
      override;
  public
    property Handle: HWND read GetIconHandle;
...
function TspTrayIcon.GetIconHandle: HWND;
begin
  Result := IconData.Wnd;
end;
...
const
   WM_CHECKLAYERBORDER = WM_USER + 402;
begin
  if Owner is TWinControl then
    if Application.MainForm <> nil
    then
      begin
        Application.MainForm.Tag := -10000;
        ShowWindow(Application.Handle, SW_RESTORE);
        Application.MainForm.Visible := True;
        if Application.MainForm.WindowState = wsMinimized
        then Application.MainForm.WindowState := wsNormal;
        PostMessage(Application.MainForm.Handle, WM_CHECKLAYERBORDER, 0, 0);
      end;
end;
...
begin
  ...
  begin
    ...
    {$IFNDEF VER200}
    StrLCopy(szInfo, PChar(Text), System.SizeOf(szInfo)-1);
    StrLCopy(szInfoTitle, PChar(Title), System.SizeOf(szInfoTitle)-1);
    {$ELSE}
    StrPLCopy(szInfo, Text, System.SizeOf(szInfo)-1);
    StrPLCopy(szInfoTitle, Title, System.SizeOf(szInfoTitle)-1);
    {$ENDIF}
