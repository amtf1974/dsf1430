...
type
  ...
  TspQuickButtonPos = (spqbpLeft, spqbpRight);
  ...
  protected
    ...
    procedure SetPopupMenu(Value: TspSkinPopupMenu); virtual;
    procedure SetCaption(Value: String); virtual;
    procedure SetPosition(Value: TspQuickButtonPos);
    procedure SetDown(Value: Boolean);
  public
    ItemRect: TRect;
    Active: Boolean;
  ...
  published
    ...
    property PopupMenu: TspSkinPopupMenu read FPopupMenu write SetPopupMenu;
    property Caption: String read FCaption write SetCaption;
    property Position: TspQuickButtonPos read
...
begin
  ...
  begin
    ...
    then
      begin
        Frame3D(Buffer.Canvas, R, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
        Brush.Color := SP_BTNDOWNCOLOR;
        FillRect(R);
      end
    else
      if FMouseIn
...
var
  ...
  CanDraw: Boolean;
begin
  if (Parent.CaptionTabs.Count <> 0) and (FTabDataIndex >= 0)
  then
    begin
  ...
  begin
    ...
    then
      ...
      begin
        ...
        if Parent.QuickButtons[i].Down and (Parent.QuickButtons[i].GroupIndex = 0) then
...
begin
  ...
  FTabDataIndex := Parent.FSD.GetIndex('captiontab');
  if FTabDataIndex = -1 then Exit;
  Buffer := TBitmap.Create;
  StartX := LX;
  Data := TspDataSkinCaptionTab(Parent.FSD.ObjectList[FTabDataIndex]);
    ...
    begin
      ...
      begin
        ...
        then
          ...
          w := 0;
        w := w + 20 + RectWidth(R);
        ItemRect := Rect(LX, Y, LX + w, Y + h);
        if ItemRect.Right > RX
        then
          begin
            ItemRect := NullRect;
            w := 0;
  ...
  case Parent.CaptionTabPos of
    sptpRight:
      begin
        OffsetX := (RX - AllTabsWidth - 10) - StartX;
        if OffsetX < 0 then OffsetX := 0;
        if OffsetX > 0 then
        for i := 0 to Parent.FCaptionTabs.Count - 1 do
          if Parent.FCaptionTabs[i].Visible then
            OffsetRect(Parent.FCaptionTabs[i].ItemRect, OffsetX, 0);
        LX := RX - AllTabsWidth - 15 - ObjectRect.Left;
      end;
    sptpCenter:
      begin
        OffsetX := (RX - StartX) div 2 - AllTabsWidth div 2;
        if OffsetX < 0 then OffsetX := 0;
        if OffsetX > 0 then
        for i := 0 to Parent.FCaptionTabs.Count - 1 do
          if Parent.FCaptionTabs[i].Visible then
...
begin
  ...
  Buffer.SetAlpha(255);
  TspColorRec(Buffer.PixelPtr[0, 2]^).A := 0;
  TspColorRec(Buffer.PixelPtr[1, 2]^).A := 40;
  TspColorRec(Buffer.PixelPtr[3, 2]^).A := 40;
  TspColorRec(Buffer.PixelPtr[4, 2]^).A := 0;
  TspColorRec(Buffer.PixelPtr[0, 1]^).A := 40;
  TspColorRec(Buffer.PixelPtr[4, 1]^).A := 40;
  Buffer.CheckRGBA2;
  bf.BlendOp := AC_SRC_OVER;
  bf.BlendFlags := 0;
  bf.SourceConstantAlpha := 255;
...
begin
  ...
  R := Rect(0, 0, 10, 10);
  SPDrawSkinText(Cnvs, S, R, DT_CALCRECT);
...
var
  X, Y: Integer;
begin
  if not (Parent.CanShowBorderLayer and Parent.FBorderLayer.FData.FullBorder) and
     not Parent.FQuickButtons[Index].Active and not Parent.FQuickButtons[Index].FDown
  then
    begin
      DrawQuickButton(Cnvs, Index);
      Exit;
    end;
...
procedure TspSkinCaptionObject.DrawQuickButtonsAlpha(Cnvs: TCanvas; var R: TRect);
...
begin
  ...
  if not IsNullRect(DividerRect) and Parent.FShowIcon and not Parent.MenuButtonVisible and
     (biSystemMenu in Parent.BorderIcons)
  then
  ...
  with Parent.FQuickButtons[i] do
   if Visible and (Position = spqbpLeft)
   then
     begin
       ItemRect := Rect(X, Y,
       ...
       Inc(X, RectWidth(ItemRect) + 2);
       Inc(R.Left, RectWidth(ItemRect) + 2);
     end;

  X := ObjectRect.Right - 5;
  for i := 0 to Parent.QuickButtons.Count - 1 do
  with Parent.FQuickButtons[i] do
   if Visible and (Position = spqbpRight)
   then
     begin
       ...
       DrawQuickButtonAlpha(Cnvs, i);
       Dec(X, RectWidth(ItemRect) + 2);
       Dec(R.Right, RectWidth(ItemRect) + 2)
     end;
end;
    ...
    begin
      ...
      begin
        ...
        then
          ...
          if (Parent.QuickButtons[i].GroupIndex = 0) then
            Parent.QuickButtons[i].FDown := False;
      end;
      UpdateNCForm;
...
begin
  ...
  ACanvas.CopyRect(
    Rect(Bounds.Left, Bounds.Top,
      Bounds.Left + B.Width, Bounds.Top + B.Height),
    B.Canvas,
...
begin
  ...
  //
  ACanvas.CopyRect(
    Rect(Bounds.Left - 6, Bounds.Top - 6,
    Bounds.Left - 6 + B.Width, Bounds.Top - 6 + B.Height),
    B.Canvas,
    Rect(0, 0, B.Width, B.Height));
  B.Free;
  B2.Free;
  ...
  begin
    ...
    then
      begin
        ...
        SPDrawSkinText(Cnv, S1, CR, DT_CALCRECT or DT_NOPREFIX);
...
begin
  ...
  then
    begin
      ...
      if not IsNullRect(DividerRect) and Parent.ShowIcon and not Parent.MenuButtonVisible and
         (biSystemMenu in Parent.BorderIcons)
      then
        begin
          DrawDivider(Cnvs,
  ...
  then
    begin
      ...
      DrawTabs(Cnvs, TextOffset, ObjectRect.Left + RealTextRect.Right,
...
begin
  ...
  then
    begin
      ...
      if not IsNullRect(DividerRect) and Parent.ShowIcon and not Parent.MenuButtonVisible
         and (biSystemMenu in Parent.BorderIcons)
      then
        begin
          DrawDivider(Cnvs,
             ObjectRect.Left + 3,
    ...
    then
      ...
      begin
        ...
        then
          begin
            MouseEnter;
            if (not DSF.InMenu) or (MenuItem.Count = 0)
            then
              begin
...
begin
  ...
  begin
    ...
    TspSkinMainMenuBarItem(ObjectList.Items[FLastItemIndex]).FDown := True;
    TspSkinMainMenuBarItem(ObjectList.Items[FLastItemIndex]).Active := True;
    TspSkinMainMenuBarItem(ObjectList.Items[FLastItemIndex]).MouseDown(0, 0, mbLeft);
    FLastItemIndex := -1;
    Exit;
  end;
  for i := 0 to ObjectList.Count - 1 do
  ...
  then
    ...
    begin
      ...
      begin
        TspSkinMainMenuBarItem(ObjectList.Items[j]).FDown := False;
        TspSkinMainMenuBarItem(ObjectList.Items[j]).FMouseIn := False;
        RePaint;
        FLastItemIndex := j;
        TrackScrollMenu;
      end  
...
begin
  ...
  begin
    ...
    if FIsW10 or (not FIsVistaOs and ((FForm.FormStyle = fsMDIForm) or
     (FForm.FormStyle = fsMDIChild))) then
...
function TspDynamicSkinForm.IsModernSkin: Boolean;
begin
  Result := False;
...
function TspDynamicSkinForm.GetNormalBoundsRect: TRect;
begin
...
begin
  ...
  Result := RectWidth(R);
  if (FMaxClientWidth > 0) and (FWindowState <> wsMaximized) and FSupportNCArea  then
  begin
...
begin
  R := GetMonitorWorkArea(FForm.Handle, not FMaximizeOnFullScreen);
  Result := RectHeight(R);
  if (FMaxClientHeight > 0) and (FWindowState <> wsMaximized) and FSupportNCArea then
  begin
    if FSkinSupport then
    begin
      if FSD.SkinnableForm
...
begin
  ...
  if not FInShortCut and (FMainMenuBar <> nil) and
     (FMainMenuBar.GetChildMainMenu <> nil) and not Handled
  then
    begin
...
function TspDynamicSkinForm.GetBoundsRect: TRect;
begin
  Result := Rect(FFormLeft, FFormTop, FFormWidth, FFormHeight);
  if CanShowBorderLayer and FBorderLayer.FVisible and not IsNullRect(FBorderLayer.FData.BorderRect) and
    FBorderLayer.FData.FullBorder then
  begin
    Dec(Result.Left, FBorderLayer.LeftSize);
    Dec(Result.Top, FBorderLayer.TopSize);
    Inc(Result.Right, FBorderLayer.RightSize + FBorderLayer.LeftSize);
    Inc(Result.Bottom, FBorderLayer.BottomSize + FBorderLayer.TopSize);
  end;
end;
...
begin
  ...
  if not (RollUpState) and FSupportNCArea and (FWindowState <> wsMaximized)
  then
    begin
      if FClientWidth > 0 then FForm.ClientWidth := FClientWidth;
      if FClientHeight > 0 then FForm.ClientHeight := FClientHeight;
    end;
end;
...
begin
  ...
  then
    ...
    begin
      ...
      if Self.IsModernSkin and not (biSystemMenu in BorderIcons) and (C.ObjectRect.Left <= 2) then
      begin
        ObjectRect := Rect(TopPosition, TopPosition,
           TopPosition + Parent.MenuButtonWidth + 5, TopPosition + RectHeight(SkinRect))
      end     
      else
...
begin
  ...
  if (not FSkinnableForm) or (not IsNullHeight and FIsW10 and (FSD <> nil) and not FSD.Empty and
...
procedure TspMDITab.DrawChildIcon(var R: TRect; Cnvs: TCanvas);
var
  FIcon: TIcon;
  IX, IY, X, Y: Integer;
...
procedure TspSkinMDITabsBar.SetShowChildIcon;
begin
...
begin
  ...
  then
    begin
      if FAddButtonActive
      then
        Cnvs.Brush.Color := SP_BTNACTIVECOLOR
      else
...
begin
  ...
  case BorderType of
    ...
    spflBottom:
     ...
     spflLeft:
      begin
        if P.X >= FForm.Left - HitLeftSize then Result := spflhLeft;
      end;
    spflRight:
      begin
        if P.X <= FForm.Left + FForm.Width + HitRightSize then Result := spflhRight;
      end;
  end;
end;
...
var
  ...
  SkinObject: TspActiveSkinObject;
begin
  inherited;
  //
  ...
  if (FLayerBorder.DSF.FForm.FormStyle = fsMDIForm) and
  ...
  then
    begin
      SkinObject := TspActiveSkinObject(FLayerBorder.DSF.ObjectList.Items[FLayerBorder.ActiveObject]);
      with SkinObject do
      begin
        FLayerBorder.MouseCaptureObject := FLayerBorder.ActiveObject;
        MouseDown(X, Y, Button);
        if (SkinObject is TspSkinCaptionObject) and
           ((TspSkinCaptionObject(SkinObject).ActiveQuickButton <> -1) or
           (TspSkinCaptionObject(SkinObject).ActiveTab <> -1))
        then
          FDown := False;
...
begin
  ...
  if (FLayerBorder.DSF.WindowState = wsNormal) and not FLayerBorder.DSF.FRollUpState and
     (biMaximize in FLayerBorder.DSF.BorderIcons) and FLayerBorder.FData.FullBorder then
  begin
    R := GetMonitorWorkArea(FForm.Handle, True);
    if Self.Top + FLayerBorder.FData.BorderRect.Top < R.Top then
    begin
      FLayerBorder.DSF.FCanMaximized := False;
      FForm.Top := R.Top + FLayerBorder.TopSize;
      FLayerBorder.DSF.WindowState := wsMaximized;
    end;
  end;
end;
...
begin
  ...
  if FDown and (OffsetY > 0) and (FLayerBorder.DSF.WindowState = wsMaximized) and
    not FLayerBorder.DSF.RollUpState and not FLayerBorder.DSF.MaximizeOnFullScreen then
  begin
    if FHitMode = spflCaption then
    begin
      //
      H := RectHeight(FLayerBorder.DSF.OldBoundsRect);
      FLayerBorder.DSF.OldBoundsRect.Top := FForm.Top;
      FLayerBorder.DSF.OldBoundsRect.Bottom := FLayerBorder.DSF.OldBoundsRect.Top + H;
      FLayerBorder.DSF.WindowState := wsNormal;
      //
      GetCursorPos(P);
  ...
  if FDown and (OffsetY <= 0) and (FLayerBorder.DSF.WindowState = wsMaximized) and (FHitMode = spflCaption) and
    not FLayerBorder.DSF.RollUpState and not FLayerBorder.DSF.MaximizeOnFullScreen then
  begin
    Exit;
  end;
  ...
  begin
    ...
    case FHitMode of
    spflCaption:
    if not  FLayerBorder.DSF.FTabChanging then
      begin
...
begin
  ...
  then
    begin
      ...
      if not IsNullRect(FData.ButtonsRect) then
        C.ObjectRect.Right := NewButtonsRect.Left - DSF.FSD.ButtonsOffset
      else
...
procedure TspQuickButtonItem.SetDown(Value: Boolean);
var
  I: Integer;
begin
   if FDown <> Value  then
  begin
    FDown := Value;
    if TspQuickButtonItems(Collection).DSF <> nil then
    with TspQuickButtonItems(Collection).DSF do
    begin
...
procedure TspQuickButtonItem.SetCaption(Value: String);
begin
  if FCaption <> Value
  then
    begin
...
procedure TspQuickButtonItem.SetPosition(Value: TspQuickButtonPos);
begin
  if FPosition <> Value
  then
    begin
...
procedure TspQuickButtonItem.SetPopupMenu(Value: TspSkinPopupMenu);
begin
  if Value <> FPopupMenu
  then
    begin
      if FPopupMenu <> nil
      then
        FPopupMenu.RemoveFreeNotification(TspQuickButtonItems(Collection).DSF);

      FPopupMenu := Value;

      if FPopupMenu <> nil then
        FPopupMenu.FreeNotification(TspQuickButtonItems(Collection).DSF);
    end;
end;
...
begin
  ...
  begin
    ...
    FPopupMenu := TspQuickButtonItem(Source).PopupMenu;
    FVisible := TspQuickButtonItem(Source).Visible;
    FPosition := TspQuickButtonItem(Source).Position;
    FGroupIndex := TspQuickButtonItem(Source).GroupIndex;
    FAllowAllUp := TspQuickButtonItem(Source).AllowAllUp;
  end
  else
