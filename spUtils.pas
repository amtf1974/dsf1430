...
uses
  ...
  ImgList, spEffBmp, ExtCtrls, CommCtrl,
  spPngImageList, spPngImage{$IFDEF VER200}, PngImage {$ENDIF};

const
  maxi = 10000;

  // colors for default draw
  SP_BTNFRAMECOLOR = 12020539;
  SP_BTNACTIVECOLOR = 15979713;
  SP_BTNDOWNCOLOR = 14990498;
  SP_GAUGEFRAME = 1290752;
...
function CalcTextWidthWNoPrefix(C: TCanvas; Str: WideString): Integer;
function CalcTextHeightWNoPrefix(C: TCanvas; Str: WideString): Integer;
...
procedure CorrectTextbyWidthWNoPrefix(C: TCanvas; var S: WideString; W: Integer);
...
procedure DrawImageAndText(Cnvs: TCanvas; R: TRect; Margin, Spacing: Integer;
            Layout: TspButtonLayout;
            Caption: String; ImageIndex: Integer; IL: TCustomIMageList; ADown: Boolean;
            AEnabled: Boolean; ADrawColorMarker: Boolean; AColorMarkerValue: TColor; AShellImage: Boolean = False);
...
var
  ...
  function GetAlternateColor(AColor: TColor): TColor;

implementation //========================================================

uses Forms, Consts, Math, SkinTabs;
...
begin
  ...
  if l < 50
  then
    Result := Lighter(AColor, 25)
  else
    Result := Darker(AColor, 10);
end;
  ...
  then
    begin
      ...
      then
        ...
        Cnvs.CopyRect(Rect(X, Y, X + Buffer.Width, Y + Buffer.Height),
...
begin
  ...
  //
  ACanvas.CopyRect(Bounds, B.Canvas, Rect(0, 0, B.Width, B.Height));
  //
  B.Free;
  B2.Free;
end;
  ...
  begin
    ...
    then
      ...
      begin
        ...
        then
          begin
            Dec(TR.Right, gw + Spacing);
          end
        else
        if (Layout = blGlyphTop) or (Layout = blGlyphBottom)
        then
          begin
            Dec(TR.Bottom, gh + Spacing);
          end;
...
begin
  ...
  then
    begin
      if AShellImage then
        ImageList_DrawEX(IL.Handle, ImageIndex, Cnvs.Handle, GX, GY, IL.Width, IL.height,
...
begin
  R := Rect(0, 0, 0, 0);
  SPDrawSkinText(C, Str, R, DT_CALCRECT or DT_NOPREFIX);
  Result := RectWidth(R);
end;
...
begin
  ...
  then
    begin
      TempR := Rect(0, 0, RectWidth(R), RectHeight(R));
      SetLength(S2, Length(S));
      S2 := Copy(S, 1, Length(S));
      DrawText(Cnvs.Handle, PChar(S2), Length(S2), TempR,
        DT_WORDBREAK or DT_CENTER or DT_CALCRECT);
      if (RectHeight(TempR) <= RectHeight(R)) and
         (RectWidth(TempR) <= RectWidth(R)) then
      begin
        DrawText(Cnvs.Handle, PChar(S2), Length(S2), R,
      ...
      begin
        ...
        S3 := S1 + '...';
        TempR := Rect(0, 0, RectWidth(R), RectHeight(R));
        DrawText(Cnvs.Handle, PChar(S3), Length(S3), TempR,
          DT_WORDBREAK or DT_CENTER or DT_CALCRECT);
        if (RectHeight(TempR) > RectHeight(R)) or
           (RectWidth(TempR) > RectWidth(R)) then
          Break;
      end;
      S4 := S4 + '...';
      DrawText(Cnvs.Handle, PChar(S4), Length(S4), R,
        DT_WORDBREAK or DT_CENTER);
    end
  else
  begin
    ...
    DrawText(Cnvs.Handle, PChar(S2), Length(S2), R, DT_WORDBREAK or DT_CENTER);
  end;
end;
...
begin
  ...
  begin
    Pen.Color := Color;
    MoveTo(X + 5, Y + 6); LineTo(X + 6, Y + 6);
    MoveTo(X + 4, Y + 5); LineTo(X + 7, Y + 5);
    MoveTo(X + 3, Y + 4); LineTo(X + 8, Y + 4);
    MoveTo(X + 2, Y + 3); LineTo(X + 9, Y + 3);
  end;
end;
...
begin
  ...
  begin
    Pen.Color := Color;
    MoveTo(X + 2, Y + 5); LineTo(X + 9, Y + 5);
    MoveTo(X + 3, Y + 4); LineTo(X + 8, Y + 4);
    MoveTo(X + 4, Y + 3); LineTo(X + 7, Y + 3);
    MoveTo(X + 5, Y + 2); LineTo(X + 6, Y + 2);
  end;
end;
...
var
  ...
  MonitorFromPoint: function(P: TPoint; dwFlags: DWORD): THandle; stdcall;
...
initialization
  ...
  begin
    ...
    MonitorFromPoint :=  GetProcAddress(User32H, 'MonitorFromPoint');
