...
type
  ...
  protected
    ...
    function GetWorkArea(P: TPoint): TRect;
...
begin
  ...
  then
   ...
   begin
     ...
     begin
       if Item2.Items[j].GroupIndex = Item.Items[i].GroupIndex
       then
         begin
           HasExist := True;
           Break;
      ...
      if not HasExist then miL.Add(Item.Items[i]);
    end;

    for i := 0 to Item2.Count - 1 do
      miL.Add(Item2.Items[I]);
    miL.Sort(@CompareValues);

   for i := StartIndex to miL.Count - 1  do
   if TMenuItem(miL.Items[i]).Visible
   then
     begin
       if TMenuItem(miL.Items[i]).Action <> nil
       then
         TMenuItem(miL.Items[i]).Action.Update;
       ItemList.Add(TspSkinMenuItem.Create(Self, TMenuItem(miL.Items[i]), DSMI));
     end;
...
function TspSkinMenu.GetWorkArea(P: TPoint): TRect;
begin
  Result := GetMonitorWorkAreaFromPoint(P, True);
end;
...
begin
  ...
  WorkArea := GetWorkArea(Point(R.Left, R.Top));
  SkinData := ASkinData;
...
begin
  ...
  then
    ...
    begin
      ...
      Frame3D(Buffer.Canvas, SknR, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
      if ASelected
      then
        Buffer.Canvas.Brush.Color := SP_BTNDOWNCOLOR
      else
        Buffer.Canvas.Brush.Color := SP_BTNACTIVECOLOR;
