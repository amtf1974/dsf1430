...
uses Windows, Messages, SysUtils, Classes, Controls, Graphics, Forms,
  Menus, StdCtrls, Buttons, SkinBoxCtrls, SkinCtrls;

type
  TspFileAttr = (spftReadOnly, spftHidden, spftSystem, spftVolumeID, spftDirectory,
    spftArchive, spftNormal);
  TspFileType = set of TspFileAttr;

  TspDriveType = (vdtUnknown, spdtNoDrive, spdtFloppy, spdtFixed, spdtNetwork, spdtCDROM,
    spdtRAM);
...
{ TspSkinFileListBox }
  ...
  published
    ...
    property FileType: TspFileType read FFileType write SetFileType default [spftNormal];
    property Font;
    property ImeMode;
    property ImeName;
...
begin
  ...
  begin
    ...
    DriveType := TspDriveType(GetDriveType(PChar(DriveChar + ':\')));
    if TextCase = tcUpperCase then
      DriveChar := Upcase(DriveChar);

    case DriveType of
      spdtFloppy:   Items.AddObject(DriveChar + ':', FloppyBMP);
      spdtFixed:    AddDrive(VolumeID(DriveChar), FixedBMP);
      spdtNetwork:  AddDrive(NetworkVolume(DriveChar), NetworkBMP);
      spdtCDROM:    AddDrive(VolumeID(DriveChar), CDROMBMP);
      spdtRAM:      AddDrive(VolumeID(DriveChar), RAMBMP);
    end;
  end;
end;
...
begin
  inherited Create(AOwner);
  Width := 145;
  FFileType := [spftNormal]; { show only normal files by default }
...
var
  ...
  SaveCursor: TCursor;
const
   Attributes: array[TspFileAttr] of Word = (faReadOnly, faHidden, faSysFile,
...
begin
  ...
  begin
    ...
    for AttrIndex := spftReadOnly to spftArchive do
      ...
      begin
        ...
        begin
          repeat            { exclude normal files if ftNormal not set }
            if (spftNormal in FileType) or (FileInfo.Attr and AttrWord <> 0) then
...
procedure TspSkinFileListBox.SetFileType(NewFileType: TspFileType);
begin
