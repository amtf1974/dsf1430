...
uses Messages, Windows, SysUtils, Classes, Graphics, Menus, Controls, Forms,
     StdCtrls, Mask, SkinData, SkinCtrls, SkinMenus;
...
type
  ...
  protected
    ...
    //
    procedure SetHScrollBar(Value: TspSkinScrollBar); virtual;
    procedure SetVScrollBar(Value: TspSkinScrollBar); virtual;
  ...
  TspGetCellParamEvent = procedure (Sender: TObject; ACol, ARow: Longint; AState: TGridDrawState;
  ...
  private
    ...
    FEditUpdate: Integer;
    FOnGetCellParam: TspGetCellParamEvent;
 ...
 published
    property FixedColSizing;
    property ColSizingWithLine;
    property OnGetCellParam: TspGetCellParamEvent
...
procedure TspSkinTransparentMaskEdit.WMCHAR;
begin
  inherited;
...
procedure TspSkinTransparentMaskEdit.Change;
begin
  inherited;
  if FTransparent then DoPaint;
end;

procedure TspSkinTransparentMaskEdit.WMSize;
begin
  inherited;
  if FTransparent then DoPaint;
end;

procedure TspSkinTransparentMaskEdit.WMLButtonDown(var Message: TMessage);
begin
  inherited;
  FDown := True;
...
procedure TspSkinTransparentMaskEdit.WMSETFOCUS;
begin
  inherited;
  if FTransparent then DoPaint;
...
procedure TspSkinTransparentMaskEdit.WMKILLFOCUS;
begin
  inherited;
  if FTransparent then DoPaint;
end;
...
procedure TspSkinTransparentMaskEdit.DoPaint2(DC: HDC); 
var
  MyDC: HDC;
  TempDC: HDC;
...
procedure TspSkinTransparentMaskEdit.DoPaint; 
var
  MyDC: HDC;
  TempDC: HDC;
  OldBmp, TempBmp: HBITMAP;
...
procedure TspSkinTransparentMaskEdit.WMPaint(var Message: TWMPaint);
var
  DC: HDC;
  PS: TPaintStruct;
begin
...
procedure  TspSkinInplaceEdit.UpdateTransparentEditor;
begin
  if FTransparent then DoPaint;
end;
...
procedure TspSkinInplaceEdit.InvalidateGrid;
var
  Cur: TRect;
begin
...
procedure TspSkinCustomGrid.SetWallPaper(Value: TBitmap);
begin
...
procedure TspSkinCustomGrid.SetWallPaperStretch(Value: Boolean);
begin
              ...
              if not UseRightToLeftAlignment
                then
                  begin
                    ...
                    if FIndex <> -1
                    then
                      Canvas.Brush.Color := FSD.SkinColors.cBtnHighLight
                    else
...
procedure TspSkinCustomGrid.WMCHECKPARENTBG;
begin
...
begin
  ...
  begin
    SSList := TStringSparseList(TSparseList(FGrid.FData)[FIndex - 1]);
    if SSList <> nil then SSList.List.ForAll({$IFNDEF WIN64}@{$ENDIF}BlankStr);
  end
  else if FIndex < 0 then
    for I := Count - 1 downto 0 do
    begin
