...
function FromRGB(Color: Cardinal): Cardinal; inline;
{ TColor -> TbsColor }
begin
  TspColorRec(Result).R := TspColorRec(Color).B;
  TspColorRec(Result).G := TspColorRec(Color).G;
  TspColorRec(Result).B := TspColorRec(Color).R;
  TspColorRec(Result).A := $FF;
end;

function ToRGB(Color32: Cardinal): Cardinal; inline;
{ TspColor -> TColor }
begin
  TspColorRec(Result).R := TspColorRec(Color32).B;
  TspColorRec(Result).G := TspColorRec(Color32).G;
  TspColorRec(Result).B := TspColorRec(Color32).R;
  TspColorRec(Result).A := TspColorRec(Color32).A;
end;
...
procedure _ClearAlpha(Src: Pointer; Count: integer; Value: Cardinal);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
    if PspColorRecArray(Src)[I].Color and not $FF000000 = Value then
      PspColorRecArray(Src)[I].A := 0;
end;

procedure _FillLongword(Src: Pointer; Count: Integer; Value: Cardinal);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
    PspColorArray(Src)[I] := Value;
end;

procedure _FillLongwordRect(Src: Pointer; W, H, X1, Y1, X2, Y2: Integer; Value: Cardinal);
var
  J: Integer;
begin
  if X2 > W then
  ...
  begin
    if Value = 0 then
      FillChar(PspColorArray(Src)[X1 + (J * W)], (X2 - X1) * SizeOf(TspColor), 0)
    else if Value = $FFFFFFFF then
      FillChar(PspColorArray(Src)[X1 + (J * W)], (X2 - X1) * SizeOf(TspColor), $FF)
    else
      _FillLongword(@PspColorArray(Src)[X1 + (J * W)], X2 - X1, Value);
  end;
end;
...
begin
  for I := 0 to Count - 1 do
    PspColorRecArray(Src)[I].A := Alpha;
end;
...
begin
  ...
  if Y1 > Y2 then
    Y1 := Y2;
  for J := Y1 to Y2 - 1 do
    _FillAlpha(@PspColorArray(Src)[X1 + (J * W)], X2 - X1, Alpha);
end;

function _PixelAlphaBlend(F, B: TspColor): TspColor; inline;
var
  FA: Byte;
  OneMinusFA: Byte;
begin
  if TspColorRec(F).A = 0 then
  begin
    Result := B;
    Exit;
  end;

  if TspColorRec(F).A = $FF then
  begin
    Result := F;
    Exit;
  end;
  ...
  Result := B;
  TspColorRec(Result).R := MulDiv(TspColorRec(F).R, FA, $FF) + MulDiv(TspColorRec(B).R, OneMinusFA, $FF);
  TspColorRec(Result).G := MulDiv(TspColorRec(F).G, FA, $FF) + MulDiv(TspColorRec(B).G, OneMinusFA, $FF);
  TspColorRec(Result).B := MulDiv(TspColorRec(F).B, FA, $FF) + MulDiv(TspColorRec(B).B, OneMinusFA, $FF);
end;
...
begin
  for I := 0 to Count - 1 do
    PspColorArray(Dst)[I] := _PixelAlphaBlend(PspColorArray(Src)[I], PspColorArray(Dst)[I]);
end;
