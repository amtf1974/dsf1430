...
begin
  if (SkinData <> nil) and (not SkinData.Empty) and
     (SkinData.GetControlIndex(FSkinDataName) <> -1)
  then
...
var
  ...
  NewClRect: TRect;
begin
  CIndex := SkinData.GetControlIndex(FSkinDataName);
...
begin
  ...
  PanelData := TspDataSkinPanelControl(SkinData.CtrlList[SkinData.GetControlIndex(FSkinDataName)]);
...
begin
  ...
  begin
    ...
    if spbdsDown in State then
    begin
      EdgeColor := SP_BTNFRAMECOLOR;
      Canvas.Brush.Color := SP_BTNDOWNCOLOR;
    ...
    begin
       EdgeColor := SP_BTNFRAMECOLOR;
      Canvas.Brush.Color := SP_BTNACTIVECOLOR;
      Canvas.Font.Color := clBtnText;
    end
    else
