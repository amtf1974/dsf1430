...
procedure TspSkinCustomEdit.CreateWnd;
begin
  inherited;
  AdjustTextRect(True);
end;
...
begin
  ...
  W := Width;
  H := Height;
  if (Items.Count > 0) and (ItemHeight > 0) then
  begin
    IH := ItemHeight;
    ...
    IH := 0;
  R := Rect(0, j * IH, W, H);
  if (RectWidth(R) > 0) and (RectHeight(R) > 0) then
  begin
    SaveIndex := SaveDC(DC);
    try
...
begin
  ...
  then
    ...
    begin
      Result := TspDataSkinEditControl(SkinData.CtrlList[i]).DisabledFontColor;
      if TspDataSkinEditControl(SkinData.CtrlList[i]).FontColor = Result
      then
        Result := clGrayText;
    end;
end;
...
begin
  for I := 0 to FWrapperList.Count - 1 do
    TspCheckListBoxDataWrapper(FWrapperList[I]).Free;
  FWrapperList.Free;
...
var
  I: Integer;
  LIndex: Integer;
  LWrapper: TspCheckListBoxDataWrapper;
begin
  for I := 0 to Items.Count - 1 do
    if HaveWrapper(I) then
    begin
      LWrapper := TspCheckListBoxDataWrapper(GetWrapper(I));
...
begin
  if HaveWrapper(Index) then
  begin
    LWrapper := TspCheckListBoxDataWrapper(GetWrapper(Index));
    LIndex := FWrapperList.IndexOf(LWrapper);
    if LIndex <> -1 then
      FWrapperList.Delete(LIndex);
    LWrapper.Free;
...
procedure TspSkinScrollBox.AlignControls(AControl: TControl; var Rect: TRect); 
begin
  inherited;
  GetVRange;
  GetHRange;
...
procedure TspSkinDateEdit.CalendarMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TspSkinMemo2.WMNCPAINT(var Message: TMessage);
begin
end;
...
var
  PLB: TspSkinCustomCheckComboBox;
  I: Integer;
  TickCount: DWORD;
  AnimationStep: Integer;
begin
  PLB := nil;
  //
  if CheckW2KWXP and (Owner is TspSkinCustomCheckComboBox)
  then
    begin
      PLB := TspSkinCustomCheckComboBox(Owner);
      if PLB.ListBoxAlphaBlend and not FOldAlphaBlend
      then
        begin
          SetWindowLong(Handle, GWL_EXSTYLE,
...
procedure TspSkinCustomCheckComboBox.DrawResizeButton;
var
  Buffer, Buffer2: TBitMap;
  CIndex: Integer;
  ButtonData: TspDataSkinButtonControl;
...
procedure TspSkinCustomCheckComboBox.DrawResizeSkinItem;
var
  Buffer: TBitMap;
  R, R2: TRect;
  W, H: Integer;
...
procedure TspSkinCustomCheckComboBox.CalcSize(var W, H: Integer);
var
  XO, YO: Integer;
begin
  if FUseSkinSize
