...
type
 ...
 TspSkinToolBar = class(TspSkinPanel)
  ...
  TspSkinCheckBox = class(TspSkinCheckRadioBox);
  TspSkinRadioButton = class(TspSkinCheckRadioBox)
    constructor Create(AOwner: TComponent); override;
  end;
  ...
  TspDrawItemImageEvent = procedure(AItem: TListItem;
  ...
  protected
    ...
    FUseSkinFont: Boolean;
    FDefaultColor: TColor;
    FSkinColor, FSkinAlternateColor: TColor;
    //
    FVScrollBar: TspSkinScrollBar;
    FHScrollBar: TspSkinScrollBar;
    ...
    FOnDrawItemImage: TspDrawItemImageEvent;
    //
    ...
    procedure NewDrawItem(Sender: TCustomListView; Item: TListItem;
      State: TCustomDrawState);

    procedure NewDefaultDrawItem(Sender: TCustomListView; Item: TListItem;
      State: TCustomDrawState);
    ...
    procedure NewCustomDrawItem(Sender: TCustomListView;
     Item: TListItem; State: TCustomDrawState; var DefaultDraw: Boolean);  virtual;
  ...
  public
    ...
    property OnDrawSkinSubItem: TspDrawSkinSubItemEvent
     ...
     property OnDrawItemImage: TspDrawItemImageEvent read
...
procedure TspSkinButton.SetTransparent(Value: Boolean);
begin
...
begin
  ...
  begin
    ...
    then
      ...
      begin
        if FTransparent and (SkinData <> nil) then
  ...
  then
    begin
      ...
      DrawImageAndText(B.Canvas, TR, FMargin, FSpacing, FLayout,
        ...
        False, Enabled, False, 0, FShellImage);
    end
  else
...
begin
  ...
  then
    begin
      Frame3D(B.Canvas, R, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
      B.Canvas.Brush.Color := SP_BTNDOWNCOLOR;
      B.Canvas.FillRect(R);
      IsDown := True;
    end
  else
    ...
    then
      begin
        Frame3D(B.Canvas, R, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
        B.Canvas.Brush.Color := SP_BTNACTIVECOLOR;
        B.Canvas.FillRect(R);
      end
    else
      begin
...
begin
  ...
  {$IFDEF VER150_UP}
  GetCursorPos(P);
  if (WindowFromPoint(P) <> Handle) and FMouseIn and FDown
  then
    Perform(CM_MOUSELEAVE, 0, 0)
  else
  if (WindowFromPoint(P) = Handle) and not FMouseIn and FDown
  then
...
procedure TspSkinMenuButton.CreateControlSkinResizeImage2;
var
  R, R1: TRect;
  isDown: Boolean;
  E: Boolean;
...
procedure TspSkinMenuButton.CreateControlSkinResizeImage;
var
  R, R1: TRect;
  isDown: Boolean;
  E: Boolean;
...
begin
  ...
  begin
    inherited;
    if (FIndex <> -1) and FTransparent and  not AMouseIn and not ADown and not IsNullRect(MenuMarkerFlatRect) then
    begin
      R := Self.NewClRect;
      R.Left := R.Right;
      R.Right := R.Left + RectWidth(MenuMarkerFlatRect);
      DrawMenuMarker(B.Canvas, R, False, False);
    end;
  end;  
end;
    ...
    begin
      ...
      then
        begin
          ...
          B.Canvas.Brush.Color := SP_BTNACTIVECOLOR;
          B.Canvas.FillRect(R);
          Frame3D(B.Canvas, R1, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
          B.Canvas.Brush.Color := SP_BTNDOWNCOLOR;
...
procedure TspSkinMenuButton.DropDown;
begin
    ...
    begin
      ...
      then
        ...
        begin
          ...
          if FMouseIn
          then
            Frame3D(Buffer.Canvas, IR, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1)
          else
...
begin
  ...
  if FMouseIn
  then
    Frame3D(B.Canvas, IR, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1)
  else
...
constructor TspSkinRadioButton.Create(AOwner: TComponent);
begin
  inherited;
  FRadio := True;
  FSkinDataName := 'radiobox';
...
begin
  ...
  then
    begin
      B.Canvas.Brush.Color := SP_GAUGEFRAME;
...
begin
  ...
  begin
    ...
    then
      begin
        Frame3D(B.Canvas, BR1, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
...
begin
  ...
  begin
    if not IsNullRect(Buttons[i].R) and PtInRect(Buttons[i].R, Point(X, Y))
    then
      begin
        j := i;
        Break;
...
begin
  ...
  if not IsNullRect(Buttons[i].R) then DrawButton(B.Canvas, i);
  if Enabled then DrawButton(B.Canvas, THUMB);
end;
    ...
    then
      ...
      begin
        ...
        begin
          ...
          then
            begin
              Frame3D(Cnvs, R1, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
              Brush.Color := SP_BTNDOWNCOLOR;
              FillRect(R1);
            end
          else
       ...
       case FKind of
         sbHorizontal:
           begin
             ...
             //
             if not IsNullRect(UpButtonRect) and (UpButtonRect.Left > RTPt.X)
             then
               OffsetRect(Buttons[UPBUTTON].R, Offset, 0);

             if not IsNullRect(DownButtonRect) and (DownButtonRect.Left > RTPt.X)
             then
         ...
         sbVertical:
           begin
             ...
             if not IsNullRect(UpButtonRect) and (UpButtonRect.Top > LBPt.Y)
             then
               OffsetRect(Buttons[UPBUTTON].R, 0, Offset);

             if not IsNullRect(DownButtonRect) and (DownButtonRect.Top > LBPt.Y)
             then
...
begin
  ...
  if RectWidth(NodeRect) * RectHeight(NodeRect) = 0 then Exit;
  ...
  begin
    ...
    then
      ...
      begin
        Treeview_GetItemRect(Self.Handle, Node.ItemId, TempNodeRect, True);
        TempNodeRect.Right := TempNodeRect.Left + Canvas.TextWidth(Node.Text) + 3;
...
begin
  ...
  FAlternateRow := False;
  FSkinColor := clWindow;
  FSkinAlternateColor := FSkinColor;
...
procedure TspSkinCustomListView.NewDefaultDrawItem(Sender: TCustomListView; Item: TListItem;
  State: TCustomDrawState);
var
  SelectBounds, TextBounds, IconBounds, ItemBounds, R, CR, R1: TRect;
  B, Buffer: TBitmap;
  CX, CY, i: Integer;
  S: String;
  w1, w2: Integer;
  ...
  then
    begin
      ...
      begin
        ...
        if Focused and (cdsFocused in State) then
         DrawFocusRect(Rect(0, 0, B.Width, B.Height));
      end;
      R := TextBounds;
     ...
     if (ViewStyle = vsReport) and (Item.SubItems.Count > 0) and RowSelect
      ...
      if FAlternateRow and (ViewStyle = vsReport)
      then
        begin
          R1 := SelectBounds;
          R1.Left := R1.Right;
          ...
          then
            ...
            Canvas.Brush.Color := GetAlternateColor(Self.Color);
          Canvas.FillRect(R1);
        end;
      B.Free;
    end
   ...
   then
    ...
    then
      begin
        ...
        if ViewStyle = vsIcon
        then
          CY := IconBounds.Bottom - Buffer.Height - 1
        else
        ...
        begin
          ...
          if Item.Checked then
            DrawCheckImage(Buffer.Canvas, 7, Buffer.Height div 2 - 4, clBlack)
        end;
...
procedure TspSkinCustomListView.NewDrawItem(Sender: TCustomListView; Item: TListItem;
   State: TCustomDrawState);
var
  SelectBounds, TextBounds, IconBounds, ItemBounds, R, CR, R1: TRect;
  B, Buffer: TBitmap;
...
begin
  ...
  then
    begin
      NewDefaultDrawItem(Sender, Item, State);
      Exit;
    end;

  ItemBounds := Item.DisplayRect(drBounds);

  if RectWidth(ItemBounds) = 0 then Exit;
  if RectHeight(ItemBounds) = 0 then Exit;
  ...
  then
    begin
      ...
      B.Height := RectHeight(SelectBounds);
      with ListBoxData do
      if Focused and (cdsFocused in State)
      then
        CreateStretchImage(B, Picture, FocusItemRect, ItemTextRect, True)
      else
        CreateStretchImage(B, Picture, ActiveItemRect, ItemTextRect, True);
     ...
     if (ViewStyle = vsReport) and (Item.SubItems.Count > 0) and RowSelect
      ...
      then
        begin
          ...
          if (Item.Index div 2) = (Item.Index / 2)
          then
            Canvas.Brush.Color := FSkinColor
          else
            Canvas.Brush.Color := FSkinAlternateColor;
          Canvas.FillRect(R1);
        end;
      B.Free;
    end
...
procedure TspSkinCustomListView.NewAdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
      State: TCustomDrawState; Stage: TCustomDrawStage; var DefaultDraw: Boolean);
begin
  if Stage <> cdPostPaint then Exit;
  DefaultDraw := False;
  NewDrawItem(Sender, Item, State);
end;
...
begin
  ...
  then
    begin
      {$IFDEF VER230_UP}
      OnAdvancedCustomDrawItem := NewAdvancedDrawItem;
      {$ELSE}
...
procedure TspSkinCustomListView.SetAlternateRow(Value: Boolean);
begin
  if Value <> FAlternateRow
  then
    begin
      FAlternateRow := Value;
      if FDrawSkin then Invalidate;
    end;
end;
...
begin
  ...
  begin
    ...
    then
      begin
        Frame3D(B.Canvas, BR, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
        Brush.Color := SP_BTNDOWNCOLOR;
      end
    else
    if Active
...
begin
  ...
  then
    ...
    begin
      Color := FDefaultColor;
      Font := FDefaultFont;
      FSkinColor := FDefaultColor;
    end;
  FSkinAlternateColor := GetAlternateColor(FSkinColor);
  if (SkinData <> nil) and (SkinData.ResourceStrData <> nil)
  then
    Font.Charset := SkinData.ResourceStrData.CharSet
  else
    ...
    begin
      ...
      then
        ...
        begin
          ...
          LRect := Rect(0, 0, NewClRect.Left, Height); 
       end;
      ...
      if FSizeable and PtInRect(RRect, P) and not FRollState and (Align = alLeft) and (not PointInRect(CR, P) )
      then
        Message.Result := HTRIGHT
      else
      if FSizeable and PtInRect(LRect, P) and not FRollState and (Align = alRight)  and (not PointInRect(CR, P) )
      then
  ...
  then
    ...
    begin
      ...
      then
        begin
          Frame3D(Cnvs, R1, SP_BTNFRAMECOLOR, SP_BTNFRAMECOLOR, 1);
          Cnvs.Brush.Color := SP_BTNDOWNCOLOR;
          Cnvs.FillRect(R1);
        end
      else
        if MouseIn
...
procedure TspSkinScrollPanel.AlignControls(AControl: TControl; var ARect: TRect);
begin
  inherited;
...
procedure TspSkinScrollPanel.UpdateNC;
begin
...
begin
  if Buttons[0].Visible <> AVisible then
  begin
...
procedure TspSkinScrollPanel.DoMouseEnter;
begin
  if (csDesigning in ComponentState) then Exit;
  FMouseIn := True;
  if CheckActivation
...
procedure TspSkinScrollPanel.DoMouseLeave;
var
  P: TPoint;
begin
  if (csDesigning in ComponentState) then Exit;
  ...
           B := False;
    ...
    WM_LBUTTONUP:
     ...
     begin
       ...
       if (((FScrollType = stVertical) and (P.Y < 0)) or
           ((FScrollType = stHorizontal) and (P.X < 0))) and Buttons[0].Visible and
           Buttons[0].MouseIn and (FCapturedButton = 1)
       then
         ButtonUp(0)
       else
       if (((FScrollType = stVertical) and (P.Y > ClientHeight)) or
          ((FScrollType = stHorizontal) and (P.X > ClientWidth))) and Buttons[1].Visible and
...
begin
  ...
  then
    ...
    begin
      ...
      if not Enabled then
        begin
          if not IsNullRect(DisabledSkinRect) then
            CreateSkinImage(LTPoint, RTPoint, LBPoint, RBPoint, CLRect,
              NewLtPoint, NewRTPoint, NewLBPoint, NewRBPoint, NewCLRect,
              B, FSkinPicture, DisabledSkinRect, B.Width, B.Height, True,
              LeftStretch, TopStretch, RightStretch, BottomStretch,
              StretchEffect, StretchType)
          else
            CreateSkinImage(LTPoint, RTPoint, LBPoint, RBPoint, CLRect,
...
procedure TspSkinToolBar.AlignControls(AControl: TControl; var ARect: TRect);
begin
  inherited;
  if not FUpdatingScrollInfo then
  begin
    if AdjustControls then
       AdjustAllControls;
    if  FCanScroll then
      GetScrollInfo;
  end;
...
procedure TspSkinToolBar.UpdateNC;
begin
  SetWindowPos(Handle, 0, Self.Left, Self.Top, Self.Width, Self.Height,
    SWP_FRAMECHANGED or SWP_DRAWFRAME or
    SWP_NOSIZE or SWP_NOMOVE or SWP_NOZORDER);
...
begin
  ...
  case Message.Msg of
    WM_NCHITTEST:
      ...
      begin
        //
        if not FMouseIn then DoMouseEnter;
        //
        P.X := LoWord(Message.lParam);
        P.Y := HiWord(Message.lParam);
        P := ScreenToClient(P);
  ...
            B := False;
    ...
    WM_NCLBUTTONDOWN, WM_NCLBUTTONDBLCLK:
     ...
     WM_LBUTTONUP:
     if FCanScroll and (FCapturedButton > 0) then
     begin
       FStartScroll := False;
       ReleaseCapture;
       Buttons[FCapturedButton - 1].Down := False;
       ...
       P.X := TWMLButtonUp(Message).XPos;
       P.Y := TWMLButtonUp(Message).YPos;
       if (P.X < 0) and Buttons[0].Visible and
           Buttons[0].MouseIn and (FCapturedButton = 1)
       then
         ButtonUp(0)
       else
       if (P.X > ClientWidth) and Buttons[1].Visible and
          Buttons[1].MouseIn and (FCapturedButton = 2)
       then
         ButtonUp(1);
       FCapturedButton := 0;
...
procedure TspSkinToolBar.DoMouseEnter;
begin
  if (csDesigning in ComponentState) then Exit;
  FMouseIn := True;
  if CheckActivation
...
procedure TspSkinToolBar.DoMouseLeave;
var
  P: TPoint;
begin
