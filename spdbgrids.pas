...
begin
  ...
  if (Key = '-') or (Key = '+') or (Key = {$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator)
  then
    begin
      case FT of
        ...
        ftFloat, ftCurrency:
          Result := not (
                    ((Key = {$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator) and
                     (Pos({$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator, Text) <> 0)) or
...
var
  ...
  begin
    ...
    begin
      InflateRect(TitleRect, 1, 1);
      Frm3D(Canvas, TitleRect, Darker(MasterCol.Title.Color, 20), Darker(MasterCol.Title.Color, 20));
    end;
...
begin
  ...
  begin
    InflateRect(ARect, 1, 1);
    Frm3D(Canvas, ARect, Darker(FixedColor, 20), Darker(FixedColor, 20));
  end;
end;
