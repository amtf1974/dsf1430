...
uses Sysutils, Classes, {$IFDEF WIN64}zlib{$ELSE}spzlib{$ENDIF};
...
begin
  ...
  CCheck(deflateInit_({$IFDEF WIN64}FZRec{$ELSE}@FZRec{$ENDIF}, Levels[CompressionLevel], zlib_version, sizeof(FZRec)));
end;
...
begin
  ...
  FZRec.avail_in := 0;
  DCheck(inflateInit_({$IFDEF WIN64}FZRec{$ELSE}@FZRec{$ENDIF}, zlib_version, sizeof(FZRec)));
end;
