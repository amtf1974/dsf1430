...
type
 ...
 public
  ...
  TspSystemColorHook = (spschHighLight, spschWindow);
  TspSystemColorHooks = set of TspSystemColorHook;
  ...
  protected
    ...
    //
    FChangeSystemColors: Boolean;
    FSystemColorHooks: TspSystemColorHooks;
    //
    ...
    procedure SetChangeSystemColors(Value: Boolean);
  public
    SizeGripArea: TRect;
  ...
  published
    ...
    property ChangeSystemColors: Boolean
      read FChangeSystemColors write SetChangeSystemColors;
    property SystemColorHooks: TspSystemColorHooks
      read FSystemColorHooks write FSystemColorHooks;
...
function bsGetActualAddr(Proc: Pointer): Pointer;
...
begin
  if Proc <> nil then
  begin
    if (Win32Platform <> VER_PLATFORM_WIN32_NT) and IsWin9xDebugThunk(Proc)
    then
...
procedure UnhookSysProc(Proc: Pointer; var BackupCode: TXRedirCode);
var
  {$IFDEF VER230_UP}
  n: NativeUInt;
  {$ELSE}
...
begin
  ...
  begin
    Proc := bsGetActualAddr(Proc);
    Assert(Proc <> nil);
    WriteProcessMemory(GetCurrentProcess, Proc,
      @BackupCode, SizeOf(BackupCode), n);
    BackupCode.Jump := 0;
  end;
end;

procedure HookSysProc(Proc, Dest: Pointer; var BackupCode: TXRedirCode);
var
  {$IFDEF VER230_UP}
  n: NativeUInt;
  {$ELSE}
...
begin
  ...
  if Proc <> nil then
    if ReadProcessMemory(GetCurrentProcess,
      Proc, @BackupCode, SizeOf(BackupCode), n)
    then
      begin
        Code.Jump := $E9;
...
var
  SysColorHooked: Boolean = False;
  SysPenHooked: Boolean = False;
  SysBrushHooked: Boolean = False;
  SysSkinData: TspSkinData = nil;

function NewCreatePen(Style, Width: Integer; Color: COLORREF): HPEN; stdcall;
begin
  if (SysSkinData <> nil) and (SysSkinData.Empty) and SysPenHooked then
  begin
    UnHookCreatePen;
  ...
  if (SysSkinData <> nil) and not (SysSkinData.Empty)
  then
  begin
    case Color of
      COLOR_WINDOW:
        if spschWindow in SysSkinData.SystemColorHooks then
          Color := ColortoRGB(SysSkinData.SkinColors.cWindow);
      COLOR_WINDOWTEXT:
        if spschWindow in SysSkinData.SystemColorHooks then
          Color := ColortoRGB(SysSkinData.SkinColors.cWindowText);
      COLOR_HIGHLIGHT:
        if spschHighLight in SysSkinData.SystemColorHooks then
          Color := ColortoRGB(SysSkinData.SkinColors.cHighLight);
      COLOR_HIGHLIGHTTEXT:
        if spschHighLight in SysSkinData.SystemColorHooks then
         Color := ColortoRGB(SysSkinData.SkinColors.cHighLightText);
    end;
  end;
  UnHookCreatePen;
  Result := CreatePen(Style, Width, Color);
...
function NewGetSysColor(nIndex: Integer): DWORD; stdcall;
begin
  if (SysSkinData <> nil) and not (SysSkinData.Empty) then
  begin
    if (nIndex = COLOR_WINDOW) and (spschWindow in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cWindow)
    else
    if (nIndex = COLOR_WINDOWTEXT) and (spschWindow in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cWindowText)
    else
    if (nIndex = COLOR_HIGHLIGHT) and (spschHighLight in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cHighLight)
    else
    if (nIndex = COLOR_HIGHLIGHTTEXT) and (spschHighLight in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cHighLightText)
    else
      begin
        UnHookGetSysColor;
  ...
  if (SysSkinData <> nil) and SysSkinData.Empty and SysColorHooked then
  begin
    UnHookGetSysColor;
    Result := GetSysColor(nIndex);
  end
...
function NewGetSysColorBrush(nIndex: Integer): HBRUSH; stdcall;
begin
  if (SysSkinData <> nil) and not (SysSkinData.Empty) then
  begin
    if (nIndex = COLOR_WINDOW) and (spschWindow in SysSkinData.SystemColorHooks)  then
      Result := CreateSolidBrush(Cardinal(ColortoRGB(SysSkinData.SkinColors.cWindow)))
    else
    if (nIndex = COLOR_WINDOWTEXT) and (spschWindow in SysSkinData.SystemColorHooks)  then
      Result := CreateSolidBrush(Cardinal(ColortoRGB(SysSkinData.SkinColors.cWindowText)))
    else
    if (nIndex = COLOR_HIGHLIGHT) and (spschHighLight in SysSkinData.SystemColorHooks)  then
      Result := CreateSolidBrush(Cardinal(ColortoRGB(SysSkinData.SkinColors.cHighLight)))
    else
    if (nIndex = COLOR_HIGHLIGHTTEXT) and (spschHighLight in SysSkinData.SystemColorHooks)  then
      Result := CreateSolidBrush(Cardinal(ColortoRGB(SysSkinData.SkinColors.cHighLightText)))
    else
      begin
  ...
  if (SysSkinData <> nil) and SysSkinData.Empty and SysBrushHooked  then
  begin
    UnHookGetSysColorBrush;
    Result := GetSysColorBrush(nIndex);
  end
...
begin
  if not SysColorHooked
  then
    HookSysProc(@Windows.GetSysColor, @NewGetSysColor, WinGetSysColor);
  SysColorHooked := True;
end;

procedure UnHookGetSysColor;
begin
  if SysColorHooked then
  begin
    SysColorHooked := False;
    UnhookSysProc(@Windows.GetSysColor, WinGetSysColor);
  end;
end;

procedure HookGetSysColorBrush;
begin
  if not SysBrushHooked then
  begin
    SysBrushHooked := True;
    HookSysProc(@Windows.GetSysColorBrush,
      @NewGetSysColorBrush, WinGetSysColorBrush);
  end;
end;
...
begin
  if SysBrushHooked
  then
    UnhookSysProc(@Windows.GetSysColorBrush, WinGetSysColorBrush);
  SysBrushHooked := False;
end;

procedure HookCreatePen;
begin
  if not SysPenHooked then
  begin
    SysPenHooked := True;
      HookSysProc(@Windows.CreatePen, @NewCreatePen, WinCreatePen);
  end;
end;
...
begin
  if SysPenHooked
  then
    UnhookSysProc(@Windows.CreatePen, WinCreatePen);
  SysPenHooked := False;
end;
...
begin
  ...
  HitTestRect := ReadRect(IniFile, 'LAYERFRAME', 'hittestrect');
...
begin
  ...
  WriteRect(IniFile, 'LAYERFRAME', 'hittestrect', HitTestRect);
...
begin
  ...
  FChangeSystemColors := False;
  FSystemColorHooks := [spschHighLight];
end;

destructor TspSkinData.Destroy;
begin
   {$IFNDEF WIN64}
  if SysSkinData = Self then
  begin
    UnHookGetSysColor;
    UnHookGetSysColorBrush;
    UnHookCreatePen;
...
procedure TspSkinData.SetChangeSystemColors(Value: Boolean);
begin
  FChangeSystemColors := Value;
  {$IFNDEF WIN64}
  if FChangeSystemColors and (SysSkinData = nil) and
     not SysColorHooked and not
    (csDesigning in ComponentState) and not Self.Empty
  then
  begin
    SysSkinData := Self;
    HookGetSysColor;
  ...
  if FChangeSystemColors and (SysSkinData <> nil) and (SysSkinData = Self) and
     not SysColorHooked and not
    (csDesigning in ComponentState) and not Self.Empty
  then
  begin
  ...
  if not FChangeSystemColors then
  begin
    if (SysSkinData <> nil) and (SysSkinData = Self) then
    begin
      UnHookGetSysColor;
      UnHookGetSysColorBrush;
      UnHookCreatePen;
...
begin
  ...
  SetChangeSystemColors(FChangeSystemColors);
  //
...
begin
  ...
  FResStrs.Add('MSG_BTN_STRETCH_HINT=Fit Picture');
  ...
  FResStrs.Add('MSG_LV_THUMBNAILS=Thumbnails');
  FResStrs.Add('MSG_LV_BIGTHUMBNAILS=Big Thumbnails');
  FResStrs.Add('MSG_LV_VERYBIGTHUMBNAILS=Very Big Thumbnails');
  ...
  FResStrs.Add('FLV_LOOKIN=Folder: ');
