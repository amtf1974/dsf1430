...
begin
  R := Rect(0, 0, 0, 0);
  SPDrawSkinText(Cnvs, S, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
  W := RectWidth(R);
  H := RectHeight(R);
  if FSD <> nil
  then
...
begin
  ...
  if FspHint.AlphaBlendSupport then
  begin
    if FspHint.AlphaBlend or (not FspHint.AlphaBlend and FOldAlphaBlend) then
  ...
  BoundsRect := Rect;
  //
  if FspHint.AlphaBlendSupport and FspHint.AlphaBlend
  then
  begin
    SetWindowLong(Handle, GWL_EXSTYLE,
      GetWindowLong(Handle, GWL_EXSTYLE) or WS_EX_LAYERED);
    if FspHint.AlphaBlendAnimation
    then
    ...
    begin
      ...
      then
        begin
          R := Rect(0, 0, 0, 0);
          Font.Style := [fsBold];
          DrawText(Handle, PChar(FspHint.FHintTitle), -1, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
          W := RectWidth(R);
          H := RectHeight(R);
          X := NewClRect.Left + 5;
          Y := NewClRect.Top;
          R := Rect(X, Y, X + W, Y + H);
          DrawText(Handle, PChar(FspHint.FHintTitle), -1, R, DT_LEFT or DT_NOPREFIX);
      ...
      //
      R := Rect(0, 0, 0, 0);
      DrawText(Handle, PChar(Caption), -1, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
      W := RectWidth(R);
      H := RectHeight(R);
      if OffsetX = 0
      then
      ...
      R := Rect(X, Y, X + W, Y + H);
      DrawText(Handle, PChar(Caption), -1, R, DT_LEFT or DT_NOPREFIX);
    end
  else
    with DrawBuffer.Canvas do
    begin
      ...
      DrawText(Handle, PChar(Caption), -1, R1, DT_LEFT or DT_NOPREFIX);
    end;
  //
  if FspHint.AlphaBlend and not FspHint.AlphaBlendSupport
  then
...
begin
  ...
  then
    ...
    begin
      ...
      {$IFDEF TNTUNICODE}
      SPDrawSkinText(DrawBuffer.Canvas, Caption, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
      {$ELSE}
      DrawText(Handle, PChar(Caption), -1, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
      {$ENDIF}
      W := RectWidth(R);
      ...
      R := Rect(X, Y, X + W, Y + H);
      {$IFDEF TNTUNICODE}
      SPDrawSkinText(DrawBuffer.Canvas, Caption, R, DT_LEFT or DT_NOPREFIX);
      {$ELSE}
      DrawText(Handle, PChar(Caption), -1, R, DT_LEFT or DT_NOPREFIX);
      {$ENDIF}
    end
